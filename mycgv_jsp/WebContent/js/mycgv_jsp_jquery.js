$(document).ready(function(){
	...
	********************************************/
	...
	/*******************************************
		회원가입 폼 - 아이디 중복 체크 
	 ********************************************/
	$("#btnIdCheck").click(function(){ // blur -> 해당 칸에 밖에 나온 경우에 실행됨
		//alert("idCheck");
		if($("#id").val() == ""){
			alert("아이디를 입력해주세요");
			$("#id").focus();
			...
			$.ajax({
				url:"idCheckProc.jsp?id="+$("#id").val(), //뒤에서 중복 체크 후 사용자에게 알려줌
				success:function(result){
					if(result == 1){
						$("#idcheck_msg").text("이미 사용 중인 아이디입니다. 다시 입력해주세요").css("color","red")
						.css("font-size","11px").css("display","block")
						.css("padding","8px 0 8px 162px");
						$("#id").val("").focus();
					}else if(result == 0){
						$("#idcheck_msg").text("사용 가능한 아이디입니디").css("color","blue")
						.css("font-size","11px").css("display","block")
						.css("padding","8px 0 8px 162px");
						$("#pass").focus();
					}
